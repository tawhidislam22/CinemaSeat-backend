...
async function check() {
    ...
    try {
        const res = await client.query('SELECT id, booking_ref, status, user_id, amount FROM bookings ORDER BY created_at DESC LIMIT 5');
        console.log("Recent Bookings:", res.rows);
    } catch (err) {
        console.error(err);
    } finally {
        await client.end();
